...
var LinkerSettingTab = class extends import_obsidian6.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian6.Setting(containerEl).setName(t("Activate virtual linker")).setDesc(t("To show/hide virtual links in the body of regular notes (paragraphs, lists, etc.), please turn on/off this toggle. Note: This toggle cannot control virtual links inside tables and Canvas (due to different rendering mechanisms). If virtual links in tables or Canvas are not displayed or show rendering glitches, do not toggle this switch \u2014 simply restart the plugin (via QuickAdd or other means).")).addToggle((toggle) => toggle.setValue(this.plugin.settings.linkerActivated).onChange(async (value) => {
    ...
    new import_obsidian6.Setting(containerEl).setName(t("Allow virtual links in headers")).setDesc(t("When enabled, virtual links will be displayed inside Markdown headings. Tip: use with Quick Switcher++ for header navigation.")).addToggle((toggle) => toggle.setValue(this.plugin.settings.allowLinksInHeaders).onChange(async (value) => {
      const update = { allowLinksInHeaders: value };
    ...
    new import_obsidian6.Setting(containerEl).setName(t("Header jump retry delay (ms)")).setDesc(t("When you click a virtual link pointing to a heading, the plugin jumps again after a short delay to correct position drift in large files. This is the base delay in milliseconds; it retries 3 times with increasing intervals. Minimum 100.")).addText((text) => text.setValue(this.plugin.settings.headerJumpRetryDelay.toString()).onChange(async (value) => {
    ...
    if (!this.plugin.settings.includeAllFiles) {
      ...
      new import_obsidian6.Setting(containerEl).setName(t("Per-note excluded keywords")).setDesc(t(`When enabled, the global excluded keywords only apply to notes that opt in via a frontmatter property. When disabled, excluded keywords apply to all notes. Usage: add "fakelink-exclude: true" to a note's frontmatter to opt in for that note.`)).addToggle((toggle) => toggle.setValue(this.plugin.settings.perNoteExcludeKeywords).onChange(async (value) => {
        await this.plugin.updateSettings({ perNoteExcludeKeywords: value });
        this.display();
      }));
      if (this.plugin.settings.perNoteExcludeKeywords) {
        new import_obsidian6.Setting(containerEl).setName(t("Frontmatter exclusion property")).setDesc(t("The frontmatter property name to check. Only notes with this property set to true will have the global excluded keywords applied. Default: fakelink-exclude.")).addText((text) => text.setValue(this.plugin.settings.frontmatterExcludeProperty).onChange(async (value) => {
    ...
    if (this.plugin.settings.advancedSettings) {
      ...
      new import_obsidian6.Setting(containerEl).setName(t("Exclude self-links to the current note")).setDesc(t('If toggled, links to the note itself are excluded from the linker. Enabling "Allow virtual links in headers" also turns this on automatically.')).addToggle((toggle) => toggle.setValue(this.plugin.settings.excludeLinksToOwnNote).onChange(async (value) => {
      ...
      new import_obsidian6.Setting(containerEl).setName(t("Enable frontmatter exclude list")).setDesc(t(`When enabled, each note can define excluded keywords in its frontmatter. These keywords will not be linked anywhere (added on top of the global excluded keywords). Usage: add "fakelink-exclude-keywords: [keyword1, keyword2]" or "fakelink-exclude-keywords: keyword1, keyword2" to a note's frontmatter.`)).addToggle((toggle) => toggle.setValue(this.plugin.settings.enableFrontmatterExcludeList).onChange(async (value) => {
        await this.plugin.updateSettings({ enableFrontmatterExcludeList: value });
        this.display();
      }));
      if (this.plugin.settings.enableFrontmatterExcludeList) {
        new import_obsidian6.Setting(containerEl).setName(t("Frontmatter exclude list property")).setDesc(t("The frontmatter property name for per-note excluded keyword lists. Default: fakelink-exclude-keywords.")).addText((text) => text.setValue(this.plugin.settings.frontmatterExcludeListProperty).onChange(async (value) => {
    ...
    new import_obsidian6.Setting(containerEl).setName(t("Header link color")).setDesc(t("Color for header virtual links (e.g., #517ea0).")).addText((text) => {
    ...
    new import_obsidian6.Setting(containerEl).setName(t("Note link color")).setDesc(t("Color for note and alias virtual links (e.g., #c0392b).")).addText((text) => {
